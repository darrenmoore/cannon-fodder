 ...
 */
/**
 * A man in the air, on his way down.
 *
 * Called instead of the whole steer-and-decide pass while `stagger` is running,
 * so anyone caught in a blast is out of the fight for a moment rather than
 * simply relocated. The velocity he was given decays rather than stopping dead,
 * and he still moves through the ordinary collision step -- so he lands against
 * the wall behind him instead of inside it.
 *
 * Returns true while he is still down.
 */
export function stumble(actor: Actor, map: GameMap, dt: number): boolean {
  if (actor.stagger <= 0) return false;
  actor.stagger -= dt;
  const damping = Math.exp(-dt * CONFIG.blast.drag);
  actor.vel.x *= damping;
  actor.vel.y *= damping;
  moveWithCollision(actor, map, dt);
  unstick(actor, map);
  // Face the way you were thrown; a man does not aim while he is tumbling.
  if (Math.hypot(actor.vel.x, actor.vel.y) > 2) {
    actor.angle = Math.atan2(actor.vel.y, actor.vel.x);
  }
  return true;
}
