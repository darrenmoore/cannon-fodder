...
import { formatTime, rankName, rankShort, rankTier } from './campaign.js';
import { button, fill, heading, meter, plate, readout } from './ui.js';
import { Phase } from './types.js';
import type { Aftermath, MissionRecord } from './campaign.js';
...
export class Hud {
  ...
  private showResult(world: World): void {
    ...
    // Offered only once there is something to visit. An empty hill is a promise
    // the game has not earned the right to make yet.
    if (this.buried > 0) {
      actions.appendChild(button('Boot Hill', {
        hint: `${this.buried} buried`, onClick: () => this.onBootHill?.(),
      }));
    }
