...
import { moveWithCollision, steer, unstick } from './steering.js';
...
/** A rooted enemy that drifted off its post walks back to it. */
function returnHome(e: Enemy): Vec2 | null {
  if (!e.home) return null;
  return Math.hypot(e.home.x - e.pos.x, e.home.y - e.pos.y) > 6 ? e.home : null;
}
